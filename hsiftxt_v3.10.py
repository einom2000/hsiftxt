...
while running:
    for role_roll in range(ROLE_TO_LOOP):
        ...
        role_running_elapsed = time.time()
        current_role_running = True
        last_anti_afk = time.time()
        hook_found = None
        while current_role_running:
            ...
            print('2 stop = :' + str(int(cur_time - running_elapsed)) + '2 stop current = ' +
                  str(int(cur_time - role_running_elapsed)) + ' & 2 act: ' +
